...
const backerData = GAMES_JSON.map((contributors) => contributors.backers);
const totalContributions = backerData.reduce((prev, next) => prev + next);
// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${totalContributions.toLocaleString("en-US")}`;
// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");

const pledgeData = GAMES_JSON.map((amount) => amount.pledged);
const totalRaised = pledgeData.reduce((prev, next) => prev + next);
...
function filterUnfundedOnly() {
  ...
  const filteredGames = GAMES_JSON.filter(
    (unfunded) => unfunded.goal > unfunded.pledged
  );
...
function filterFundedOnly() {
  ...
  const filteredGames = GAMES_JSON.filter(
    (funded) => funded.goal <= funded.pledged
  );
...
unfundedBtn.addEventListener("click", filterUnfundedOnly); // 7
fundedBtn.addEventListener("click", filterFundedOnly); // 4
allBtn.addEventListener("click", showAllGames); // 11
